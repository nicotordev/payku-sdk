...
describePaykuIntegration("integration / wallet", () => {
  ...
  test("lists wallet movements", async () => {
    const payku = createSandboxChileClient();
  ...
  test("creates and retrieves a sandbox wallet payout using approved amount", async () => {
    const payku = createSandboxChileClient();
    const approvedAmount = PAYKU_WALLET_SANDBOX_AMOUNTS.APPROVED[0]; // 1000

    const payoutResult = await runCapabilityTest("wallet.payouts.create", () =>
    ...
    if (payoutResult.status === "skipped") {
      expect(payoutResult.reason).toContain("skipped: capability unavailable");
      return;
    }
