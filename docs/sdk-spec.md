...
- **Cliente por país**: `Payku.forCountry("CL" | "PE" | "VE")` fija currency y tipa solo los módulos soportados; el modo global `new Payku()` sigue disponible. `isSupported(feature)` consulta esa matriz sin lanzar.
- **Helpers híbridos**: `sign`, `paymentMethods.toSlug` / `resolve` y `Payku.isError` conviven con `buildSign`, `paymentMethodToSlug`, `resolvePaymentMethod` e `isPaykuError`.
